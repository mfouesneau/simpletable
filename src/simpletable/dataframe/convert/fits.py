...
try:
    ...
    def fits_generate_header(
        df: pd.DataFrame, header_info: HeaderInfo | None = None
    ) -> fits.Header:
        ...
        if header_info is not None:
            info.header.update(header_info.header)
            info.alias.update(header_info.alias)
            info.units.update(header_info.units)
            info.comments.update(header_info.comments)
    ...
    def fits_generate_hdu(
        df: pd.DataFrame, index: bool = True, header_info: HeaderInfo | None = None
    ) -> BinTableHDU:
        ...
        BinTableHDU
            The generated HDU.
        """
        if header_info is None and hasattr(df, "header"):
            header_info = df.header
    ...
    def to_fits(
        ...
        **kwargs: dict
        ...
        """
        hdu = fits_generate_hdu(df, index=index, header_info=header_info)
    ...
    to_fits = None  # pyright: ignore[reportAssignmentType]
