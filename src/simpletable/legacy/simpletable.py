...
def isinstance_FITS_rec(object):
    ...
    if pyfits is None:
        return False
    return isinstance(object, pyfits.FITS_rec)


try:
    import tables  # pyright: ignore[reportMissingImports]
except ImportError:
    tables = None
...
try:
    from .plotter import Plotter  # pyright: ignore[reportMissingImports]
except ImportError:
    Plotter = None
...
def _fits_read_header(hdr):
    ...
    """
    if pyfits is None:
        raise ImportError("pyfits is not installed")
...
def _fits_generate_header(tab):
    ...
    hdr = pyfits.header.Header(cards)
...
def _fits_writeto(
    ...
    hdu.writeto(
        filename, overwrite=clobber, output_verify=output_verify, checksum=checksum
    )
...
def _ascii_read_header(
        ...
        if line[0] == comments:  # header part
            if len(line) > 2:
                if line[1] == comments:  # column meta data
                    ...
                    k = line[1:].split("\t")
                    if len(k) > 1:
                        key = str(k[0].strip())  # remove trainling spaces
                        val = " ".join(k[1:]).strip()
    ...
    for name in names:
        try:
            _names.append(name.decode())  # pyright: ignore[reportAttributeAccessIssue]
...
def _hdf5_write_data(
    ...
    **kwargs,
    ...
    """
    if tables is None:
        raise ImportError("The 'pytables' module is required for HDF5 support")
...
class SimpleTable(object):
    ...
    def __init__(self, fname, *args, **kwargs):
        ...
        if (isinstance(fname, (dict, tuple, list, types.GeneratorType))) or (
            dtype in [dict, "dict"]
        ):
            try:
                self.header = fname.pop("header", {})  # pyright: ignore
            ...
            if (extension == "csv") or dtype == "csv":
                ...
                if _pd is not None:  # pandas is faster
                    ...
                    self.data = _pd.read_csv(fname, *args, **kwargs).to_records()  # pyright: ignore
                else:
                    kwargs.setdefault("skip_header", n)
                    kwargs.setdefault("comments", "#")
                    self.data = np.genfromtxt(fname, delimiter=",", *args, **kwargs)
                self.header = header
                self._units.update(**units)
                self._desc.update(**comments)
                self._aliases.update(**aliases)
                kwargs.setdefault("names", True)
            elif (extension in ("tsv", "dat", "txt")) or (
                dtype in ("tsv", "dat", "txt")
            ):
                ...
                self._units.update(**units)
                self._aliases.update(**aliases)
            elif (extension in ("hdf5", "hd5", "hdf")) or (
                dtype in ("hdf5", "hd5", "hdf")
            ):
                if tables is None:
    ...
    def pprint_entry(self, num, keys=None):
        ...
        """
        if (keys is None) or (keys == "*"):
            _keys = list(self.keys())  # pyright: ignore
        elif type(keys) in basestring:
            _keys = [
                k
                for k in (list(self.keys()) + list(self._aliases.keys()))  # pyright: ignore
        ...
        **kwargs,
        ...
        if (fields is None) or (fields == "*"):
            ...
            if "," in fields:
                ...
                _fields = [fields]
            lbls = self.keys() + tuple(self._aliases.keys())  # pyright: ignore
            _keys = []
            for _fk in _fields:
    ...
    def to_dict(self, keys=None, contiguous=False):
        ...
        if keys is None:
            _keys = list(self.keys())  # pyright: ignore
        if contiguous:
            return {k: np.ascontiguousarray(self[k]) for k in _keys}
        return {k: self[k] for k in _keys}
    ...
    def _clean_orphan_aliases(self):
        ...
        self._aliases = {
            k: v for k, v in self._aliases.items() if v in self.data.dtype.names
        }
    ...
    def resolve_alias(self, colname):
        ...
        """
        # User aliases
        if hasattr(colname, "__iter__") & (not isinstance(colname, basestring)):
    ...
    def keys(self, regexp=None, full_match=False, skip_aliases=False):
        ...
        if (regexp is None) or (regexp == "*"):
            ...
            lbls: tuple[str, ...] = self.colnames  # pyright: ignore
            if not skip_aliases:
    ...
    def take(self, indices, axis=None, out=None, mode="raise"):
        ...
        """
        return self.data.take(indices, axis, out, mode)  # pyright: ignore
    ...
    def __setitem__(self, k, v):
        if k in self:
            ...
            object.__setitem__(self, k, v)  # pyright: ignore
    ...
    def info(self):
        ...
        vals = [
            (k, self._units.get(k, ""), self._desc.get(k, ""))
            for k in self.colnames or []
        ]
        lengths = [
    ...
    def __getslice__(self, i, j):
        return self.data.__getslice__(i, j)  # pyright: ignore
    ...
    def stack(self, r, *args, **kwargs):
        ...
        sdata = recfunctions.stack_arrays(
            data, defaults, usemask=False, asrecarray=True
        )
    ...
    def join(
        ...
        if left_on is None and right_on is None:
            ...
            if np.ma.isMaskedArray(left_values):
                mask = ~left_values.mask  # pyright: ignore
                left_values = left_values.data  # pyright: ignore
            ...
            if np.ma.isMaskedArray(right_values):
                mask = ~right_values.mask  # pyright: ignore
    ...
    def add_column(self, name, data, dtype=None, unit=None, description=None):
        ...
        if len(self.data.dtype) > 0:  # pyright: ignore
    ...
    def evalexpr(self, expr, exprvars=None, dtype=float):
        ...
        """
        _globals = {}
        for k in list(self.colnames or []) + list(self._aliases.keys()):
            if k in expr:
                _globals[k] = self[k]
...
class AstroTable(SimpleTable):
    ...
    def where(self, condition=None, condvars=None, cone=None, zone=None, **kwargs):
        ...
        if zone is not None:
            if len(zone) != 4:
                msg = (
                    "Expecting zone keywords as a tuple of 4 elements:"
        ...
        if (cone is not None) and (zone is not None):  # cone + zone
            ...
            ind = ind & self.zoneSearch(zone[0], zone[1], zone[2], zone[3], outtype=2)  # pyright: ignore
            d = d[ind]  # pyright: ignore
            blobs.append(d)
        elif cone is not None:
            ra, dec, r = cone
            _ind, d = self.coneSearch(ra, dec, r, outtype=2)
            ind = ind & _ind.astype(bool)
            blobs.append(d[ind])  # pyright: ignore
        elif zone is not None:
            _ind = self.zoneSearch(zone[0], zone[1], zone[2], zone[3], outtype=1)
            ind = ind & _ind  # pyright: ignore
    ...
    def selectWhere(
        ...
        """
        ind, blobs = self.where(condition, condvars, cone, zone, **kwargs)  # pyright: ignore
        tab = self.select(fields, indices=ind)
